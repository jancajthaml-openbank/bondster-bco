...
class UnitHelper
  ...
  def download()
    ...
    side_image_name = "bondster_bco_artifacts#{image_version}"
    arch = ENV['UNIT_ARCH']
    ...
    begin
      file.write([
        "FROM alpine",
        "COPY --from=openbank/bondster-bco:#{image_version} /opt/artifacts/bondster-bco_#{debian_version}_#{arch}.deb /tmp/artifacts/bondster-bco.deb",
        "ENTRYPOINT /bin/ls",
    ...
    FileUtils.mv('/tmp/artifacts/bondster-bco.deb', '/tmp/packages/bondster-bco.deb')

    raise "no package to install" unless File.file?('/tmp/packages/bondster-bco.deb')
  end

  def prepare_config()
    config = Array[@@default_config.map {|k,v| "BONDSTER_BCO_#{k}=#{v}"}]
    config = config.join("\n").inspect.delete('\"')
