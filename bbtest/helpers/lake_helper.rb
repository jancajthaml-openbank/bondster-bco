...
module LakeMock
  ...
  def self.parse_message(msg)
    ...
    if groups = msg.match(/^Wall\/bbtest BondsterUnit\/([^\s]{1,100}) token ([^\s]{1,100}) TN$/i)
    ...
    elsif groups = msg.match(/^Wall\/bbtest BondsterUnit\/([^\s]{1,100}) token ([^\s]{1,100}) TD$/i)
      tenantId, reqId = groups.captures
    ...
    elsif groups = msg.match(/^Wall\/bbtest BondsterUnit\/([^\s]{1,100}) token ([^\s]{1,100}) EE$/i)
      tenantId, reqId = groups.captures
