...
class UnitHelper(object):
  ...
  def download(self):
    ...
    package = 'bondster-bco_{}_{}'.format(self.debian_version, self.arch)
    scratch_docker_cmd.append('COPY --from={} /opt/artifacts/{}.deb /tmp/packages/bondster-bco.deb'.format(image, package))
    ...
    try:
      ...
      tar_stream, stat = self.docker.get_archive(scratch['Id'], '/tmp/packages/bondster-bco.deb')
      with open(tar_name.name, 'wb') as destination:
        for chunk in tar_stream:
          destination.write(chunk)

      archive = tarfile.TarFile(tar_name.name)
      archive.extract('bondster-bco.deb', '/tmp/packages')

      (code, result, error) = execute([
        'dpkg', '-c', '/tmp/packages/bondster-bco.deb'
      ])
