...
class UnitHelper(object):
  ...
  def default_config():
    return {
      ...
      "HTTP_PORT": "443",
      "SERVER_KEY": "/etc/bondster-bco/secrets/domain.local.key",
      "SERVER_CERT": "/etc/bondster-bco/secrets/domain.local.crt",
  ...
  def install(self, file):
    ...
    if code != 'OK':
      ...
      with open('{}/reports/blackbox-tests/meta/debian.bondster-bco.txt'.format(cwd), 'w') as fd:
        fd.write(result)
  ...
  def download(self):
    ...
    self.binary = '{}/packaging/bin/bondster-bco_{}_{}.deb'.format(cwd, self.debian_version, self.arch)
    ...
    image = 'docker.io/openbank/bondster-bco:{}'.format(self.image_version)
    package = '/opt/artifacts/bondster-bco_{}_{}.deb'.format(self.debian_version, self.arch)
  ...
  def __get_systemd_units(self):
    ...
    result = [item for item in result if "bondster-bco" in item and not item.endswith('unit.slice')]
    return result
