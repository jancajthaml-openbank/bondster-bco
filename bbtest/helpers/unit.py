...
class UnitHelper(object):
  ...
  def download(self):
    ...
    package = Package('bondster-bco')

    cwd = os.path.realpath('{}/../..'.format(os.path.dirname(__file__)))

    assert package.download(version, meta, '{}/packaging/bin'.format(cwd)), 'unable to download package bondster-bco'

    self.binary = '{}/packaging/bin/bondster-bco_{}_{}.deb'.format(cwd, version, Platform.arch)
