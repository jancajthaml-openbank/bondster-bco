...
def offboard_unit(context, tenant):
  ...
  (code, result, error) = Shell.run(['journalctl', '-o', 'cat', '-u', 'bondster-bco-import@{}.service'.format(tenant), '--no-pager'])
  if code == 'OK' and result:
    with open(logfile, 'w') as f:
      f.write(result)
  Shell.run(['systemctl', 'stop', 'bondster-bco-import@{}.service'.format(tenant)])
  (code, result, error) = Shell.run(['journalctl', '-o', 'cat', '-u', 'bondster-bco-import@{}.service'.format(tenant), '--no-pager'])
  if code == 'OK' and result:
    with open(logfile, 'w') as fd:
      fd.write(result)
  Shell.run(['systemctl', 'disable', 'bondster-bco-import@{}.service'.format(tenant)])
...
def onboard_unit(context, tenant):
  (code, result, error) = Shell.run(["systemctl", 'enable', 'bondster-bco-import@{}'.format(tenant)])
  assert code == 'OK', str(result) + ' ' + str(error)
  (code, result, error) = Shell.run(["systemctl", 'start', 'bondster-bco-import@{}'.format(tenant)])
  assert code == 'OK', str(result) + ' ' + str(error)
