...
def offboard_unit(context, tenant):
  ...
  if code == 0 and result:
    with open('reports/blackbox-tests/logs/bondster-bco-import.{}.log'.format(tenant), 'w') as f:
      f.write(result)
  ...
  (code, result, error) = execute(['journalctl', '-o', 'cat', '-u', 'bondster-bco-import@{}.service'.format(tenant), '--no-pager'])
  if code == 0 and result:
    with open('reports/blackbox-tests/logs/bondster-bco-import.{}.log'.format(tenant), 'w') as fd:
      fd.write(result)
