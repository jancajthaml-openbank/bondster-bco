...
def offboard_unit(context, tenant):
  (code, result, error) = execute([
    'journalctl', '-o', 'short-precise', '-t' 'bondster-bco-import', '-u', 'bondster-bco-import@{}'.format(tenant), '--no-pager'
  ])
  if code == 0:
    with open('/tmp/reports/blackbox-tests/logs/bondster-bco-import.{}.log'.format(tenant), 'w') as f:
      f.write(result)

  execute([
    'systemctl', 'stop', 'bondster-bco-import@{}.service'.format(tenant)
  ])
  ...
  execute([
    'systemctl', 'disable', 'bondster-bco-import@{}.service'.format(tenant)
  ])
...
def onboard_unit(context, tenant):
  execute([
    "systemctl", 'enable', 'bondster-bco-import@{}'.format(tenant)
  ])
  (code, result, error) = execute([
    "systemctl", 'start', 'bondster-bco-import@{}'.format(tenant)
  ])
  assert code == 0
...
@given('bondster-bco is configured with')
