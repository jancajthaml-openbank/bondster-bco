...
pipeline {
    ...
    stages {
        ...
        stage('Setup') {
            steps {
                script {
                    ...
                    env.PROJECT_DESCRIPTION = "OpenBanking Bondster Market Connector service"
        ...
        stage('Package Docker') {
            steps {
                script {
                    DOCKER_IMAGE = docker.build("${env.ARTIFACTORY_DOCKER_REGISTRY}/docker-local/openbank/bondster-bco:amd64-${env.VERSION}.jenkins", dockerOptions())
                }
            }
        }
