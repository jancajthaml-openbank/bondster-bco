...
func LoadTokens(storage localfs.Storage, tenant string) ([]model.Token, error) {
	path := "t_" + tenant + "/import/bondster/token"
	...
	for _, id := range tokens {
		token := HydrateToken(storage, tenant, id)
		if token != nil {
...
func HydrateToken(storage localfs.Storage, tenant string, token string) *model.Token {
	if tenant == "" || token == "" {
		return nil
	}
	path := "t_" + tenant + "/import/bondster/token/" + token + "/value"
	...
	entity.ID = token
	if entity.Deserialize(data) != nil {
		return nil
	}
	return entity
}
