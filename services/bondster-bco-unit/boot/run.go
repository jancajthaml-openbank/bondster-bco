...
func (app Application) WaitReady(deadline time.Duration) (err error) {
	...
	go func() {
		...
		select {
		case <-app.actorSystem.IsReady:
			wg.Done()
			ticker.Stop()
			return
		case <-ticker.C:
			panic("actorSystem was not ready within 5 seconds")
		}
	}()
	...
	go func() {
		...
		select {
		case <-app.bondster.IsReady:
			wg.Done()
			ticker.Stop()
			return
		case <-ticker.C:
			panic("fio import was not ready within 5 seconds")
		}
	}()
