...
bundle-binaries-%: %
	@docker-compose run --rm package --arch linux/$^ --pkg bondster-bco-rest
	@docker-compose run --rm package --arch linux/$^ --pkg bondster-bco-import
...
.PHONY: bbtest
bbtest:
	@(docker rm -f $$(docker ps -a --filter="name=bondster_bco_bbtest_amd64" -q) &> /dev/null || :)
	@docker exec -it $$(\
		docker run -d -ti \
			--name=bondster_bco_bbtest_amd64 \
